...
def StoreList(data, page, command):
    ...
    for x in range(startingitem, closingitem):
        ...
        if item.setting == "Disabled" and not MySet.StoreListShowDisabled:
            continue
        message += "{0} ({1}, {2}). ".format(item.name, item.ID, item.cost)
    message += "Page {0}/{1}".format(page, page_max)
    ...
    if item is not None:
        ...
        # ItemEditType not recognised
            ...
            message = "[{0} edit <itemID> <DataType> <#>]. <DataType> can be an item's name, cost," \
                      "permission, cooldown, code, or sound".format(MySet.command)
            SendResp(data, message)
            return
...
class Settings:
    ...
    def __init__(self, system, settings_file=None):
        if settings_file and os.path.isfile(settings_file):
            ...
            self.helpmessageBuy = "{0} -> Use [{1} buy #] to purchase an item. If the item has the 'contribute' type," \
                                  "you can put an amount to pay after specifying the item number"
            ...
            self.helpmessageAdd = "[{1} add <ItemType> <cost/default> <ItemName>]. Add an item for viewers " \
                                  ...
                                  "existing item in the store. Use {0} help add [general/once/code/contribute/unique]" \
